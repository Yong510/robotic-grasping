...
INPUT_SIZE = 300
OUTPUT_DIR = 'results/10_samples_epoch28'
NUM_SAMPLES = 10
...
print(f"数据集总样本数: {len(dataset)}，验证集样本数: {len(val_indices)}")

for i in range(min(NUM_SAMPLES, len(val_indices))):
    idx = val_indices[i]
...
print(f"\n✅ 完成！共生成 {min(NUM_SAMPLES, len(val_indices))} 张图片，保存在 {OUTPUT_DIR}")
